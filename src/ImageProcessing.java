...
public class ImageProcessing {
	...
	public static void main(String[] args) {
		...
		String path = "/Users/beemihae/Desktop/ICTM/groundfloor1_robot.jpg"; // path
																				// from
																				// original
																				// picture
		String dstPathSobel = "/Users/beemihae/Desktop/ICTM/filtered.jpg"; // path
																			// you
																			// want
																			// to
																			// write,
	...
	public static List<Point[]> DetectObjects(String path, float[] RobotLocation) {
		...
		for (int i = 0; i < contours.size(); i++) {
			...
			if (rect.size.area() < maxArea && rect.size.area() > minArea && (Math.hypot(robot.center.x - rect.center.x,
				...
				rectangle_approx.add(corners1);

				// neem marge rond object: KALIBREER
				rect = new RotatedRect(rect.center, new Size(rect.size.width * 1.2, rect.size.height * 1.2),
						rect.angle);
				rect.points(corners1);
				rectangle_approx.set(counter, corners1);

				// teken toegevoegde rechthoek in blauw
				System.out.println("rechthoek " + counter);
				System.out.println(rectangle_approx.get(counter)[0]);
				Imgproc.circle(image_orig, rectangle_approx.get(counter)[0], 3, new Scalar(255, 0, 0), 10);
				Imgproc.circle(image_orig, rectangle_approx.get(counter)[1], 3, new Scalar(255, 0, 0), 10);
				Imgproc.circle(image_orig, rectangle_approx.get(counter)[2], 3, new Scalar(255, 0, 0), 10);
				Imgproc.circle(image_orig, rectangle_approx.get(counter)[3], 3, new Scalar(255, 0, 0), 10);
				for (int j = 0; j < 4; j++) {
					if (j < 3) {
						Imgproc.line(image_orig, rectangle_approx.get(counter)[j], rectangle_approx.get(counter)[j + 1],
								new Scalar(255, 0, 0), 2);
					} else {
						Imgproc.line(image_orig, rectangle_approx.get(counter)[j], rectangle_approx.get(counter)[0],
								new Scalar(255, 0, 0), 2);
					}
