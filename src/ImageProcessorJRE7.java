...
public class ImageProcessorJRE7 {
	public String pathOriginal = "C:\\Users\\gebruiker\\Documents\\GitHub\\ICTM2A\\src\\Original_picture.jpg";
	public String pathFilteredGray = "C:\\Users\\gebruiker\\Documents\\GitHub\\ICTM2A\\src\\filtered_gray.jpg";
	public String pathFilteredColor = "C:\\Users\\gebruiker\\Documents\\GitHub\\ICTM2A\\src\\filtered_color.jpg";
	private Mat originalPicture;
	private Mat originalGray;
	private Mat filteredGray;
	private Mat filteredColor;
	private double width = 1340;      //width of biggest square, needed to calibrate the screen
	private double height = 1960;
	...
	public ImageProcessorJRE7 () {
		System.out.println("iets");
		System.loadLibrary(Core.NATIVE_LIBRARY_NAME);
		originalPicture = Highgui.imread(pathOriginal);
		originalGray = Highgui.imread(pathOriginal, Imgproc.COLOR_RGB2GRAY); 
		...
		for (int i = 0; i < source.size(); i++) {
			System.out.println("\nx"+(i+1)+": "+source.get(i).x);
			System.out.println("\ny"+(i+1)+": "+source.get(i).y);
		}
	...
	public ArrayList<float[][]> getContours() {
		...
		List<Point[]> rectangle_approx2 = DetectObjects(pathFilteredGray,robotLocation);
		for (int i = 0; i < rectangle_approx2.size(); i++) {
			float[][] contour = new float[rectangle_approx2 .get(i).length][2];
			for (int j = 0; j < rectangle_approx2.get(i).length; j++) {
				contour[j][0]=(float) rectangle_approx2.get(i)[j].x;
				contour[j][1]=(float) rectangle_approx2.get(i)[j].y;				
			}
			res.add(contour);
		}
		return res;
	...
	*/
	public Mat applyFilters(String path, String dstPath, double width, double height) {
		Mat image = Highgui.imread(path, Imgproc.COLOR_RGB2GRAY);  

		// Mat imgDst = new Mat(image.size());     
		Mat imgDst = Highgui.imread(path);     
		...
		Highgui.imwrite(dstPath, imgDst1);
	...
	public  void applyFilters(double width, double height) {
		/**
		 * Generates internal objects Mat filteredGray and Mat filteredColor and writes them to their respective paths. 
		 */
		...
		Highgui.imwrite(pathFilteredGray, filteredGray);     
		Highgui.imwrite(pathFilteredColor, filteredColor);
	...
	public float [] GetRobotLocation(String path){
		...
		Mat imgHSV = Highgui.imread(path);
	...
	public List<Point[]> DetectObjects(String path,float[] RobotLocation){
  		System.out.println("Start Object Detection");
		
		//inlezen
		Mat image_orig = filteredColor;
		...
		*/
		...
		Highgui.imwrite(path, image_orig);
