...
public class getPicture {
	...
	public static Mat applySobel(String path, String dstPath) {
		...
		Mat imgDst1 = applyContours(imgDst);
		System.out.println("Sobel done");
		Imgcodecs.imwrite(dstPath, imgDst1);
	...
	public static Mat applyContours(Mat image) {
		...
		Mat imgDst = fourPointTransformation(image, source, 10, 10);
	...
	public static Mat fourPointTransformation(Mat image, ArrayList<Point> original, double widthImg, double lengthImg) {
		...
		// destination.get(0).y = (destination.get(2).x -
		// destination.get(1).x)*ratio + destination.get(1).y;
		...
		// RectF perspectiveCorrectedBoundingRect = new
		// RectF(mCrop.getPerspectiveCorrectedBoundingRect());
		...
		Imgproc.resize(image, imgDst, imgDst.size());
		return imgDst;
	};
